...
* Important remark for *Windows* users: Windows users are strongly advised to download *Git* (https://git-for-windows.github.io/) and to work from the *Git bash*. They are also advised to download `cURL` for Windows (https://curl.haxx.se/download.html). Additionally, you will need to set ```export MSYS_NO_PATHCONV=1``` at the beginning of your *Git bash* session to disable the default path mangling mechanism which interferes with OpenWhisk package naming.
